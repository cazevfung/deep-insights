...
set INSTALL_SCRIPT=%SCRIPT_DIR%install_dependencies.py
...
if exist "%SCRIPTS_DIR%\kill_all_servers.ps1" (
    echo Running comprehensive cleanup script...
    powershell -ExecutionPolicy Bypass -NoProfile -File "%SCRIPTS_DIR%\kill_all_servers.ps1"
    if errorlevel 1 (
...
if exist "%REPO_ROOT%\backend\kill_backend.py" (
    echo.
    echo Trying alternative cleanup method using Python script...
    %PYTHON_CMD% "%REPO_ROOT%\backend\kill_backend.py" 2>nul
    if errorlevel 1 (
...
if %START_SERVER%==1 if !HAS_START_ARG!==0 (
    %PYTHON_CMD% "%INSTALL_SCRIPT%" %* --start
) else (

            ...
            if platform.system() == 'Windows':
                ...
                try:
                    ...
                    if check_backend_health(port=backend_port, timeout=60):
                        ...
                        print_info("  OK  - 'LinkFormatterService initialized successfully'")
                        print_info("  FAIL - 'Failed to initialize LinkFormatterService: ...'")
                except Exception as e:
            ...
            if platform.system() == 'Windows':
                ...
                try:
                    ...
                    if not node_modules.exists() or not check_npm_dependencies_installed(client_dir):
                        ...
                        print_info("Run: python scripts/install/install_dependencies.py")
                        return processes
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python scripts/install/install_dependencies.py              # Install dependencies only
  python scripts/install/install_dependencies.py --start      # Install, start servers, and open browser
  python scripts/install/install_dependencies.py --start --no-browser  # Start servers without opening browser
  python scripts/install/install_dependencies.py --start --restart-backend  # Restart backend (stop existing, start new)
  python scripts/install/install_dependencies.py --start --backend-only --restart-backend  # Restart only backend with health check
  python scripts/install/install_dependencies.py --no-start   # Install without starting servers
        """
    )
    parser.add_argument(
        '--start',
    ...
    if should_start:
        ...
        print_info("  python scripts/install/install_dependencies.py --start --backend-only --restart-backend")

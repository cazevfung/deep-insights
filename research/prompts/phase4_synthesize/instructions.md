...
以我和你共同的出发点，并基于以下获取到的信息，运用金字塔原理，撰写能清晰回答我们共同出发点的完整文章。仅输出 Markdown 正文（禁止输出 JSON 或额外说明）。
...
`{outline_json}`
  ...
  - Phase 3 的未解问题 `{phase3_open_questions}` 需要在文中适当位置提及或说明。
  - Phase 3 的争议点 `{phase3_counterpoints}` 需要在文中进行比较分析。
...
# 简要自检
- 是否覆盖所有组成问题？每个组成问题是否在相应章节中得到明确回答？
- 每个章节是否体现了多个步骤之间的深度关联与洞察，而非简单复述？
- 关键结论、风险、争议与假设是否明确标注证据来源（`[EVID-##]`）？
- 是否提及了 Phase 3 中识别出的未解问题和争议点？
- 正文信息量是否充足回答问题？

# 输出格式

仅返回 Markdown 正文，不额外解释，也不要输出 JSON。

...
4. **install_dependencies.py**: Ports 3001 and 3000 hardcoded in multiple places
   - `stop_backend_server(port=3001)`
   - `check_backend_health(port=3001)`
   - `start_servers()` references to ports 3001 and 3000
5. **install_dependencies.bat**: Calls Python script (no hardcoded values, but depends on script)
...
### 6. Update `install_dependencies.py`
...
### 7. Update `install_dependencies.bat`
...
1. Update `install_dependencies.py` to use config
